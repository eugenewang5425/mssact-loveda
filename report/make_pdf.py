...
MUST_CONTAIN = [
    ...
    "σ_seed = 0.0050", "0.71 倍", # 种子方差实测与消融散布之比
    "杠杆排序",                    # 全文最凝练的结论
    "3.2–3.8σ_diff", "更优的工程取舍", "4.4×",   # 裁剪策略的效应量与取舍
    ...
    "组内同质性守卫",              # 自动拦截混合配置的守卫
    "3.8σ",                       # 复现后从零对比可分辨
    "σ_diff",                     # √2σ 判据口径（审核 Q-26 修正）
    "4.20σ_diff",                 # 整图协议去 Transformer
    "1.95σ_diff",                 # 修复后底去 Transformer（降为弱）
    "0.0097",                     # Δ 极差（原误作 0.008）
    "组内同质性守卫",              # 混合配置的自动拦截
    "空间强自相关",                # 泄漏边界声明
    # ---- 章节存在性 ----
