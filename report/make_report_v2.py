...
def _sec_seed():
    ...
    for g, r in groups.items():
        ...
        if g.startswith("参照"):
            continue
        # 注意: seed_variance.json 里的键是 "std"（打印时才标为 σ_seed）
        if "std" in r:
...
def _sec_budget():
    """§5.10 预算攻击（数据驱动, 来源 FACTS.arch_experiments.rows 的 lgR_b15_*）"""
    AE = FACTS.get("arch_experiments") or {}
    rows = AE.get("rows") or {}
    b = {k: v for k, v in rows.items() if k.startswith("lgR_b15_")}
    if not b:
        return "**（预算攻击结果待补齐）**"
    NAME = {"full": "MSSACT-Net 完整（6.10M）", "noecsam": "MSSACT-Net w/o ECSAM（6.04M）",
    ...
    for k, v in sorted(b.items(), key=lambda kv: -(kv[1].get("kappa") or 0)):
        m = k.replace("lgR_b15_", "")
        d = (f"{v['kappa']-ref:+.4f}" if (v.get("kappa") is not None and ref) else "—")
        out.append(f"| {NAME.get(m, m)} | {v.get('params_M')}M | **{v['kappa']:.4f}** | {d} |")
    return chr(10).join(out)


BUDGET_TBL = _sec_budget()

def _sec_scratch():
    """§5.6.6 从零训练 vs 预训练主干（数据驱动, 来源 FACTS.scratch_vs_pretrained）"""
    SP = FACTS.get("scratch_vs_pretrained") or {}
    m = SP.get("main_from_scratch") or {}
    rows = SP.get("rows") or []
    if not m:
        return "**（从零对照结果待补齐）**"
    L = ["**主数据集（60 轮，同为从零训练，同管线 P-MEM-ROLL 同协议）**", "",
         "| 模型 | 参数量 | Kappa | Δ | σ_seed 倍数 | 判读 |",
         "|---|---:|---:|---:|---:|---|"]
    ours, scr = m.get("mssact_6p10M"), m.get("deeplab_39p69M_scratch")
    if ours is not None:
        L.append(f"| **MSSACT-Net（自研）** | 6.10M | **{ours:.4f}** | — | — | 参照 |")
    if scr is not None:
        d = m.get("delta")
        sg = m.get("sigma_units")
        v = "**不可分辨**" if (sg is not None and sg < 2) else "可分辨"
        L.append(f"| DeepLabV3+ **从零训练** | 39.69M | **{scr:.4f}** | {d:+.4f} | "
                 f"{sg:.2f}σ | {v} |")
    if rows:
        L += ["", "**预训练的独立贡献**（同一模型、同一管线，仅差权重初始化）", "",
              "| 设置 | 预训练主干 | 从零训练 | **预训练贡献** |", "|---|---:|---:|---:|"]
        for r in rows:
            pre = f"{r['pretrained']:.4f}" if r.get("pretrained") is not None else "—"
            g = f"**{r['pretrain_gain']:+.4f}**" if r.get("pretrain_gain") is not None else "—"
            L.append(f"| {r['setting']} | {pre} | {r['scratch']:.4f} | {g} |")
    return chr(10).join(L)
...
| 对比实验（7 个基线） | ✅ 完成 |
| 消融实验（8 个变体） | ✅ 完成（散布 0.006–0.008 Kappa；**实测 σ_seed=0.0050，故不可分辨**，见 5.7） |
| 标签质量专项验证 | ✅ 完成（一致性分析 + 边界分层） |
| 训练策略探索 | ✅ 完成（阶段 A 11 组含 SGDR；**阶段 C 裁剪策略见 5.8**） |
| 架构改进实验（D1–D4） | ✅ 完成（同管线同协议；见 5.6.9） |
| 数据量阶梯（250/500/1000） | ✅ 完成（见 5.9） |
| 预算攻击（15 轮） | ✅ 完成（见 5.10） |
| **随机种子噪声底线** | ✅ 完成（σ_seed = 0.0050，见 5.7） |
| **裁剪策略（随机/固定、256/384/512）** | ✅ 完成（见 5.8） |
| **DeepLabV3+ 从零训练对照** | ✅ 完成（推翻原 +0.0720 的架构归因，见 5.6.6） |
| **架构缺陷数值检验（D1/D2/D3）** | ✅ 完成（见 5.6.2–5.6.4） |
| 等效性检验（TOST） | 🔄 材料已就绪（`eval_rigor.py`），待补齐 |
...
**错误一：公平性核验本身是错的。** 见下方"核验更正"表——`weights=None` 不足以
关闭主干预训练（`weights_backbone` 另有默认值）。

**错误二（更关键）：把 +0.0720 归因于架构。** 加入从零训练的 DeepLabV3+ 对照后，
该差距**基本消失**：
...
1. **从零训练的公平对比下，自研 6.10M 与 DeepLabV3+ 39.69M 不可分辨**
   （Δ = +0.0039，**0.78σ_seed**）——而后者参数是前者的 **6.5 倍**。
   初稿"DeepLabV3+ 显著优于自研模型（+0.0720）"的表述**不成立**：
   那 0.0720 中约 **95%** 来自预训练主干，而非架构。
2. **预训练主干是本任务上最大的单项杠杆**：同一模型仅切换权重初始化，
   贡献 **+0.0535 ~ +0.1540** Kappa（阶梯三档实测）。作为对照，5.6.5 中
   **全部模块消融都 ≤1.13σ_seed（≈0.0057）**，5.8 中裁剪策略为 0.0199–0.0223。
   **即：换主干权重 >> 改训练策略 > 增删模块。**
3. 这也解释了初稿为何会误判：预训练带来的巨大增益被错误地记在了
   "DeepLab 的架构（低层跳连、空洞卷积）"账上。

**核验更正（2026-09-13）**：

初稿称"已核对代码，`deeplabv3_resnet50(weights=None)`… **全部从零训练**"。
**该核验是错的，`weights=None` 并不足以关闭主干预训练。**
torchvision 的 `deeplabv3_resnet50` 有**两个互相独立**的权重参数：
`weights`（整体/分割头）与 `weights_backbone`（主干），**后者默认即
...
| **DeepLabV3+（加对照前）** | **0.2574** | **加载了 ImageNet 预训练主干** |

（`resnet18` / `resnet50` 没有 `weights_backbone` 参数，故 `weights=None` 确实等于
从零训练；混淆**仅**存在于分割工厂 `deeplabv3_resnet50`。权重文件缓存时间 Sep 6
早于 DeepLab 基线开训时间 Sep 11，佐证其确实被加载。）

**已在代码中显式区分**：`DeepLabV3Plus(pretrained_backbone=...)`。
既有 `deeplab*` 结果为 `True`（预训练）；从零对照为 `_scr` 后缀
（`lgR_deeplab_scr`、`lgR_n*_deeplab_scr`），即上表所用。
...
1. ~~**随机种子方差尚未测量**~~ → **已于 2026-09-13 实测完成，见 5.7 节**。
   同管线内 4 次运行（seed=42/7/2024/31337）给出 **σ_seed = 0.0050**，极差 **0.0094**；
   而本节同管线模块消融的总散布为 **0.0082**——**消融散布仅为种子极差的 0.87 倍**。
   故"模块差异落在噪声内"现已由**实测**支持，不再是推断。
   仍须保留的限定：n=4 时 σ 本身有不确定性（文献建议 ≥10 个种子），
   该数值适用于**量级判断**（差异是 1σ 还是 5σ），不宜作精确检验。
...
> ⚠️ **重要限定（见 5.6.6 的更正）**：下表中 DeepLabV3+ 有**两行**——
> `预训练主干`（旁证，**非协议内**）与 `从零训练`（协议内）。**只有从零那一行
> 可参与架构比较**。预训练在低数据档的优势尤其大（n250：预训练 0.2825 vs 从零 0.1762）。
>
> **纳入从零对照后的完整图景**（自研 6.10M vs DeepLabV3+ 39.69M **从零**）：
>
> | 设置 | 自研 | DeepLab从零 | 差 | 谁更高 |
> |---|---:|---:|---:|---|
> | PNG 60 轮（主协议） | 0.6312 | 0.6281 | +0.0031 | 自研 |
> | memmap 60 轮 | 0.6320 | 0.6359 | −0.0039 | DeepLab |
> | 预算攻击 15 轮 | 0.4899 | 0.4682 | **+0.0217** | **自研** |
> | 阶梯 n250 | 0.0322 | 0.1762 | −0.1440 | DeepLab |
> | 阶梯 n500 | 0.3312 | 0.4393 | −0.1081 | DeepLab |
> | 阶梯 n1000 | 0.5085 | 0.4771 | +0.0314 | 自研 |
>
> **结论（诚实版）**：**主协议（60 轮全量数据）下两者不可分辨**（|Δ| ≤ 0.78σ）；
> 两个 60 轮结果一正一负却都落在 1σ 内，这本身就是"该量级差异不可分辨"的直接证据。
> **预算受限时自研更高**（+0.0217，4.3σ）；**数据极少时（n250/n500）自研明显更低**
> ——说明自研模型**更依赖数据量**，与本节第 1 条（小数据下模块越多越差）方向一致。
...
**动机**：5.6 节"模块无贡献"的结论是在 **60 轮充足预算**下得到的。一个自然的质疑是：
**预算太足**，使各变体都有机会收敛到同一水平，从而掩盖了模块本身的差异。
本实验把预算压到 **15 轮**（pt15），其余协议不变，检验模块差异是否在预算紧张时显现。

{BUDGET_TBL}

**结论：压到 15 轮后，模块之间的差异依然很小**——完整模型 0.4899、
w/o EMR 0.4790、U-Net 0.4884，散布仅 **0.0109**（约 2σ_seed）；
"预算掩盖模块差异"这一质疑**不成立**。

**但本实验揭示了一个跨四组实验一致的模式**：

| 设置 | 完整模型 | w/o ECSAM | 差值 |
|---|---:|---:|---:|
| 预算攻击（15 轮） | 0.4899 | **0.5092** | **+0.0193** |
| 数据阶梯 n=250 | 0.0322 | **0.1164** | +0.0842 |
| 数据阶梯 n=500 | 0.3312 | **0.3954** | +0.0642 |
| 数据阶梯 n=1000 | 0.5085 | **0.5234** | +0.0149 |

**在数据或预算受限的四种设置下，移除坐标注意力（ECSAM）都更好，4/4 一致。**
这与 5.6.1 的观察吻合——ECSAM 仅占 **0.90%** 参数，且被放在编码器深层
（位置信息已被 8× 下采样稀释）。
而在 60 轮 + 全量数据（1,768 张）时，w/o ECSAM 反而略低
（0.6272 vs 0.6312，−0.0040，**<1σ，不可分辨**）。
