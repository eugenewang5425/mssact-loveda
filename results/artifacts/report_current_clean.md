...
**0.4065** 提升至
**0.6312**，OA 提升至 **0.7030**。

**2. 全部五个设计模块在数据充足时均无可测量的独立贡献。** 8 个消融变体的
Kappa 落在 0.627–0.637，
与完整模型（0.6312）的最大差异仅 **0.006**；
...
（2.4M–39.6M）逐像素分析显示：模型间一致率 **0.7589**
显著高于模型与人工标签的一致率 **0.6495**；多模型共识
与人工标签仅 **0.6815** 一致；且**容量-性能相关性
...
| 数据筛选与划分 | ✅ no-data ≤10% 筛选 → MD5 去重 → 8:1:1（train 1768 / val 221 / test 221 / test_clean 141） |
...
| 模型 | 参数量 | Kappa | OA | 最优轮/总轮 |
|---|---:|---:|---:|---|
| **MSSACT-Net light（自研）** | **6.10M** | **0.6312** | 0.7030 | 54/60 |
| **DeepLabV3+（从零，协议内）** | 39.69M | 0.6281 | 0.7018 | 60/60 |
| DeepLabV3+（预训练主干，**旁证**） | 39.69M | 0.6876 ⚠️非协议内 | 0.7497 | 60/60 |
| FPN-Seg | 27.20M | 0.6151 | 0.6912 | 60/60 |
| FCN | 23.78M | 0.6144 | 0.6903 | 60/60 |
| U-Net | 2.45M | 0.6054 | 0.6833 | 60/60 |
| Swin-Unet-Lite | 32.67M | 0.5859 | 0.6693 | 60/60 |
| PSPNet | 13.60M | 0.5175 | 0.6160 | 60/60 |
| SegFormer-Lite | 1.23M | 0.3043 | 0.4441 | 60/60 |
...
**方法**：在 test_clean（141 张）上用 6 个模型
...
| 4 | 边界像素占比（**已修正**） | 边界(距离0) 仅 **2.4%**；≤8px **21.6%**；内部(>8px) **78.4%** | 标签**并不碎片化**；聚合指标 78.4% 权重来自内部像素 |
...
**主数据集（60 轮，同为从零训练，同管线 P-MEM-ROLL 同协议）**

| 模型 | 参数量 | Kappa | Δ | σ_seed 倍数 | 判读 |
|---|---:|---:|---:|---:|---|
| **MSSACT-Net（自研）** | 6.10M | **0.6320** | — | — | 参照 |
| DeepLabV3+ **从零训练** | 39.69M | **0.6359** | +0.0039 | 0.78σ | **不可分辨** |

**预训练的独立贡献**（同一模型、同一管线，仅差权重初始化）

| 设置 | 预训练主干 | 从零训练 | **预训练贡献** |
|---|---:|---:|---:|
| 主数据集 60 轮 | — | 0.6359 | — |
| 阶梯 n250 | 0.2825 | 0.1762 | **+0.1063** |
| 阶梯 n500 | 0.4928 | 0.4393 | **+0.0535** |
| 阶梯 n1000 | 0.6310 | 0.4771 | **+0.1539** |
...
| 管线 | 变体 | 配置 | 参数量 | Kappa | 轮次 | 相对基线 |
|---|---|---|---:|---:|---|---:|
| P-MEM-ROLL | **完整模型（基线）** | — | 6.102M | 0.6320 | 60/60 | +0.0000 |
| P-MEM-ROLL | **D1 联合消融** | −FPN −Transformer −Adapter | 2.005M | 0.6293 | 60/60 | -0.0028 |
| P-MEM-ROLL | **D2 解码器坐标注意力** | ECSAM 移到上采样路径 | 6.115M | 0.6375 | 60/60 | +0.0055 |
| P-MEM-ROLL | **D3 超轻量通道** | `[16,32,64,128]` | 1.547M | 0.6103 | 60/60 | -0.0217 |
| P-MEM-ROLL | **D4 大容量通道** ⚠️ | `[64,128,256,512]`（24.24M，750 s/轮） | 24.242M | 0.6365 | 60/60 | +0.0044 |
| P-PNG | D1 联合消融 | −FPN −Transformer −Adapter | 2.005M | 0.6277 | 60/60 | -0.0035 |
| P-PNG | D2 解码器坐标注意力 | ECSAM 移到上采样路径 | 6.115M | 0.6341 | 60/60 | +0.0029 |
| P-PNG | D3 超轻量通道 | `[16,32,64,128]` | 1.547M | 0.6119 | 60/60 | -0.0193 |
| P-PNG | D4 大容量通道 | `[64,128,256,512]` | 24.242M | 0.5696 | 23/60 ⚠️partial(23/60) | -0.0616 |

> `P-MEM-ROLL` = 回退管线（memmap + 全局RNG增强 + workers=0），`P-PNG` = 正式对比/消融所用管线。两者仅数据投递后端不同，实测效应约 +0.001，故可**互相印证**但**不可混合求统计量**。
> ⚠️ 标记表示该变体仍在训练中（轮次未满），其 Kappa 为当前最优值，尚未定论。

**D4（大容量通道）的解读约束**：D4 的成本极高（**750 s/轮**，约为完整模型的 17 倍，单项占架构队列总时长的 42%）。更重要的是，它是在**架构缺陷尚未修复**的前提下做的容量对照（见 5.6.2/5.6.4：FPN 的 29.96% 参数不参与前向、Transformer 无位置感知）。
因此 D4 的结论只能解读为“**在这套（有缺陷的）实现下，加宽通道能否获益**”，**不能**解读为“该架构的容量上限”或“模块设计的优劣”。若要评估真实容量上限，须先完成 5.6.10 的 P0/P1 修复后重跑。
...
> ⚠️ 其中 DeepLabV3+ 组用的是**预训练主干**版本（`sd*_deeplab`，属代码 bug
> 的产物，见 5.6.6），与本项目「从零训练」的协议不一致，仅作 σ 的旁证参考；
> σ_seed 的主要依据是**从零训练**的 MSSACT-Net 组（n=4）。

| 组（同管线内） | seed 数 | mean | **σ_seed** | range | 区间 |
|---|---:|---:|---:|---:|---|
| 完整模型 MSSACT 6.10M (P-MEM-ROLL) | 4 | 0.6362 | **0.0050** | 0.0094 | [0.6318, 0.6412] |
| DeepLabV3+ 39.69M (P-MEM-ROLL) | 3 | 0.6931 | **0.0034** | 0.0066 | [0.6895, 0.6961] |

**采用各管线内最大 σ_seed = 0.0050（保守）**

| 管线 | 变体 | Kappa | ΔKappa | σ 倍数 | 判读 |
|---|---|---:|---:|---:|---|
| P-MEM-ROLL | `lgR_D3_ch_tiny` | 0.6103 | -0.0217 | -4.35 | **可分辨（>2σ）** |
| P-PNG | `nd_abl_bilinear` | 0.6369 | +0.0056 | +1.13 | 弱差异（1–2σ） |
| P-MEM-ROLL | `lgR_D2_decoder_ca` | 0.6375 | +0.0055 | +1.09 | 弱差异（1–2σ） |
| P-PNG | `nd_abl_trans4l` | 0.6362 | +0.0050 | +1.00 | 不可分辨（|Δ|<1σ） |
| P-MEM-ROLL | `lgR_D4_ch_large` | 0.6365 | +0.0044 | +0.89 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_no_emr` | 0.6355 | +0.0043 | +0.86 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_no_ecsam` | 0.6272 | -0.0041 | -0.81 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_no_adapter` | 0.6342 | +0.0030 | +0.59 | 不可分辨（|Δ|<1σ） |
| P-MEM-ROLL | `lgR_D1_join_nofpn_notrans` | 0.6293 | -0.0028 | -0.55 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_trans6l` | 0.6335 | +0.0023 | +0.45 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_no_trans` | 0.6302 | -0.0011 | -0.21 | 不可分辨（|Δ|<1σ） |
| P-PNG | `nd_abl_no_fpn` | 0.6314 | +0.0002 | +0.03 | 不可分辨（|Δ|<1σ） |

> **判读**：把 5.6 节的消融散布（0.006 量级）与 σ_seed = 0.0050 比较。若散布 < 1σ，则「移除模块无影响」与「模块确有微小影响但被噪声掩盖」两者**不可区分**——这正是需要 TOST 等效性检验的原因。

**重要限定**：n 仅 3–4 个种子时 σ 本身有较大不确定性（GEO-Bench 建议 ≥10）。故本表用于**量级判断**（0.006 究竟是 1σ 还是 5σ），不宜作为精确检验。另：`nd_deeplab` 属 P-PNG 世代，**不得**与 `sd*_deeplab`（P-MEM-ROLL）混合求均值（跨管线 Kappa 不可比）。
...
| 配置 | crop | 位置 | 每样本像素 | Kappa | 相对基准 | 秒/轮 | 总时长 |
|---|---:|---|---:|---:|---:|---:|---:|
| `lgR_bs8_full_lr2e4` | 256 | 随机 | 1.00× | **0.6320** | +0.0000 | 44 s | 44.0 min |
| `lgR_c256_center` | 256 | 固定中心 | 1.00× | **0.6128** | -0.0192 | 44 s | 44.0 min |
| `lgR_c384_rand` | 384 | 随机 | 2.25× | **0.6543** | +0.0223 | 100 s | 100.0 min |
| `lgR_c512_rand` | 512 | 随机 | 4.00× | **0.6524** | +0.0204 | 192 s | 192.0 min |

| 对比 | ΔKappa | σ_seed 倍数 | 计算代价 | 判读 |
|---|---:|---:|---|---|
| 256 随机 vs **256 固定中心** | -0.0192 | 3.8σ | 同代价（同尺度同像素） | **可分辨** |
| **384 随机** vs 256 随机 | +0.0223 | 4.5σ | 2.3× 于基准 | **可分辨** |
| **512 随机** vs 256 随机 | +0.0204 | 4.1σ | 4.4× 于基准 | **可分辨** |

**判读要点**：

0. **本节三个对比的效应量，全部大于 5.6 节的所有模块消融。** |ΔKappa| 为 0.0199–0.0223（**4.0–4.5σ_seed**，σ_seed=0.0050，见 5.7）；而模块消融全部 ≤1.13σ。也就是说：**在这套模型上，训练策略（随机裁剪、裁剪尺度）的收益是可分辨的，模块增删则不可分辨。** 这对后续工作更有指导意义——先选对训练策略，再谈架构。
1. **256 随机 vs 256 固定中心**：两者像素量与尺度完全相同，差异**只能**归因于「每轮随机位置」这一增强，是隔离该增强贡献的干净对照，结果 −0.0192（随机裁剪本身值 +0.0192）。
2. **256 → 384 → 512**：384 达 0.6543（+0.0223），512 为 0.6519（+0.0199）——**再放大到 512 无额外收益，代价却从 100 s/轮升到 192 s/轮**（4.4× 于基准）。故 384 是更优的工程取舍：既印证了「过小的尺度没有意义」，也说明**过大的尺度不划算**。
3. **注意 batch 随面积下调**（256²:8 / 384²:4 / 512²:2）以避免显存溢出，故每轮耗时与总时长必须一并报告——大裁剪的计算代价是平方级增长的。
4. 用户明确排除过小尺度（如 128），故不设该档；阶段 A 的历史结果显示128²（GSD 2.4m 匹配 GF-1）在本任务上几乎学不动（Kappa 0.0098）。
...
| 模型 | 参数量 | Kappa(max) | 末轮 | e50rel | e90rel | auc/max |
|---|---:|---:|---:|---:|---:|---:|
| DeepLabV3+（39.69M，**预训练主干**） | — | **0.2825** | 0.2387 | 24 | 29 | 0.289 |
| U-Net（2.45M） | — | **0.1915** | 0.1915 | 14 | 28 | 0.468 |
| DeepLabV3+（39.69M，**从零训练**） | — | **0.1762** | 0.1762 | 24 | 30 | 0.187 |
| MSSACT-Net w/o ECSAM（6.04M） | — | **0.1164** | 0.1079 | 23 | 29 | 0.282 |
| MSSACT-Net 完整（6.10M） | — | **0.0322** | 0.0261 | 27 | 28 | 0.231 |

**n = 500 张**

| 模型 | 参数量 | Kappa(max) | 末轮 | e50rel | e90rel | auc/max |
|---|---:|---:|---:|---:|---:|---:|
| DeepLabV3+（39.69M，**预训练主干**） | — | **0.4928** | 0.4928 | 16 | 27 | 0.458 |
| DeepLabV3+（39.69M，**从零训练**） | — | **0.4393** | 0.4393 | 21 | 26 | 0.356 |
| U-Net（2.45M） | — | **0.4050** | 0.4050 | 19 | 28 | 0.451 |
| MSSACT-Net w/o ECSAM（6.04M） | — | **0.3954** | 0.3954 | 19 | 29 | 0.393 |
| MSSACT-Net 完整（6.10M） | — | **0.3312** | 0.3312 | 25 | 28 | 0.235 |

**n = 1000 张**

| 模型 | 参数量 | Kappa(max) | 末轮 | e50rel | e90rel | auc/max |
|---|---:|---:|---:|---:|---:|---:|
| DeepLabV3+（39.69M，**预训练主干**） | — | **0.6310** | 0.6303 | 13 | 21 | 0.599 |
| MSSACT-Net w/o ECSAM（6.04M） | — | **0.5234** | 0.5234 | 15 | 24 | 0.522 |
| MSSACT-Net 完整（6.10M） | — | **0.5085** | 0.5085 | 17 | 25 | 0.458 |
| U-Net（2.45M） | — | **0.4913** | 0.4913 | 12 | 20 | 0.619 |
| DeepLabV3+（39.69M，**从零训练**） | — | **0.4771** | 0.4771 | 13 | 21 | 0.554 |
...
| 模型 | 参数量 | Kappa（15 轮） | 相对完整模型 |
|---|---:|---:|---:|
| DeepLabV3+（39.69M，预训练主干，**旁证**） | 39.692M | **0.6358** | +0.1459 |
| MSSACT-Net w/o ECSAM（6.04M） | 6.047M | **0.5092** | +0.0193 |
| MSSACT-Net 完整（6.10M） | 6.102M | **0.4899** | +0.0000 |
| U-Net（2.45M） | 2.446M | **0.4884** | -0.0015 |
| MSSACT-Net w/o EMR（6.10M） | 6.102M | **0.4790** | -0.0109 |
| DeepLabV3+（39.69M，**从零，协议内**） | 39.692M | **0.4682** | -0.0217 |
