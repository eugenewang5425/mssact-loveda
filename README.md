...
**修复的性能效果：跳连有害（被否决），位置编码弱正向**（120 轮同协议，σ_seed = 0.0042）：
...
5/8 可分辨**。σ_seed 本身也是协议相关的（val 0.0042 / test 0.0034，均为 **n=10** 实测）。
...
**诚实声明**：上表 ΔKappa 均来自**单次运行**（seed=42），但**种子方差已实测到文献建议的规模**：
完整模型 **n=10**（seed 42/7/2024/31337 + 1234/5555/8888/31415/27182/9999），
val 协议 σ_seed = **0.0042**（2σ = 0.0084）、整图协议 σ_seed = **0.0034**；
DeepLabV3+ 从零 n=6（σ=0.0041）、预训练主干 n=3（σ=0.0034）。
**σ 是协议相关也是配置相关的**，判定必须取同协议同配置的值。

**⚠️ 一处数据完整性事故（已修）**：DeepLab 的 9 次种子运行混了两种配置——旧 3 次用的是
ImageNet 预训练主干（BN 首层均值 0.248），新 6 次是从零（0.973）。并成一组会把
"配置差 0.065"当成种子噪声，得到 σ=**0.0334**（真实约 0.003，**差 11 倍**）；而旧的取 σ
规则是"各管线取最大（保守）"，于是这个假 σ 会被拿去给 MSSACT 消融定门限，
**使一切真实差异都判成"不可分辨"**。已修三处：按预训练状态拆分 DeepLab、
σ 改为取"与消融同配置"的组、并在 `seed_variance.py` 加入**组内同质性守卫**
（BN 统计检出手干预训练状态、参数量指纹检出架构不一致，命中即拒绝该 σ 并告警，
已用受控测试验证）。

**新结论（复现带来的）**：从零对比此前只有单次对单次（+0.0039，0.78σ，"不可分辨"）。
以 n=10 vs n=6 为准，**自研 6.099M 优于 DeepLabV3+ 39.64M +0.0082 = 3.8σ**；
选择偏差核验显示 DeepLab 偏离更大（+0.0038 vs +0.0026），校正后差距只增不减。
