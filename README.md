...
> 📄 **项目报告**：[项目报告_20260915.pdf](reports/项目报告_20260915.pdf)（48 页）——
> 含全部实验、图表与公式；[HTML 版](reports/项目报告_20260915.html)（可再生）
>
...
### ⚠️ 而"消融平坦"这个核心结论本身**依评估协议而变**

修好整图推理链后（此前有两个不兼容的缓存布局，导致 14+ 个 tag 从未进入整图协议），
第一次能在 **1024² 整图滑窗协议**上核对同一组消融。结果与 val 协议**大面积不符**：

| 变体 | val 协议 Δ（底 0.6312，σ=0.0050） | 整图协议 Δ（底 0.5774，σ=0.0041） |
|---|---:|---:|
| 去掉 EMR | **+0.0043** | −0.0049 |
| 去掉 FPN | **+0.0002** | −0.0064 |
| 去掉 Adapter | **+0.0030** | −0.0107 |
| 双线性上采样 | **+0.0057** | −0.0090 |
| Transformer 2→4 层 | **+0.0050** | −0.0102 |
| Transformer 2→6 层 | **+0.0023** | −0.0033 |
| 去掉 ECSAM | −0.0040 | **−0.0262（6.4σ）** |
| 去掉 Transformer | −0.0010 | **−0.0202（4.9σ）** |

**8 项里有 6 项符号相反。** val 协议下 6/8 是"移除反而更好"；整图协议下 **8/8 全为负、
5/8 可分辨**。σ_seed 本身也是协议相关的（val 0.0050 / test 0.0041，均为 n=4 实测）。

⇒ **"消融平坦、模块无可测量贡献"是 val 协议下的结论，不能推广到整图协议。**
在整图协议下 **Transformer 的贡献是可分辨的**（−0.0202 = 4.9σ），FPN 仍不可分辨。
"占参数 67% 的两个模块都无贡献"应**收窄**为"FPN 在两种协议下均不可分辨；
Transformer 在整图协议下可分辨"。

**缺陷诊断与它并列，不互相取消**：D1（归零 FPN 侧向支路输出逐位不变）、D3（置换等变
7.15e-07）是**与协议无关**的实现层事实。故"消融平坦"有**两个**成因：架构缺陷 **+**
val 协议对该类差异不敏感。**任何"无差异"表述都必须注明协议**。

**协议解剖（实测）已定位主因**：两种协议的推理窗口**都是 256²**（更正此前"跨尺度泛化"
的说法）——差异在图像集、覆盖率、窗口混合。用 test_clean 图像 + 与 val **完全相同**的
4 角裁剪流程重评，消融 Δ 立即呈真实负值（w/o trans −0.0168、w/o ecsam −0.0282），
与整图协议一致而远离 val ⇒ **符号翻转由图像集驱动**；val 侧的正 Δ 全部 ≤1.2σ（噪声）。
512² 窗口只有 +0.003~+0.008 的次要提升。⇒ 消融读数应以 test_clean 型协议为准。
（`results/artifacts/protocol_dissection.json`）
...
│
├── reports/                    # ★ 报告交付件
│   ├── 项目报告_20260915.pdf    #   当前报告（48 页；只保留最新一版，旧版进 backups/）
│   └── 项目报告_20260915.html   #   HTML 版（可再生，不入库）
├── figures/                    # 报告图（入库）
│
└── results/                    # ★ 全部产物集中于此
