...
for t in ["nd_unet", "nd_pspnet", "nd_fcn", "nd_deeplab", "nd_segformer",
    ...
    reg(t, "C-final/对比实验", "run_queue_new.py", "DATA_NEWSPLIT2", 60, 20, 8, 2e-4, _C_NOTE)
for t in ["nd_abl_no_emr", "nd_abl_no_ecsam", "nd_abl_no_fpn", "nd_abl_no_trans",
    ...
    reg(t, "C-final/消融实验", "run_queue_new.py", "DATA_NEWSPLIT2", 60, 20, 8, 2e-4, _C_NOTE)
reg("full_all_v2", "C-final/主模型", "run_queue_new.py", "DATA_NEWSPLIT2", 60, 20, 8, 2e-4,
    "完整模型基线 = 消融参考系")
reg("post_all_v2", "C-final/主模型", "run_queue_new.py", "DATA_NEWSPLIT2", 30, 12, 8, 5e-5,
    "微调自 full_all_v2")
...
reg(["strat_mssact_full30m_oc60", "strat_mssact_sgdr120", "strat_mssact_oc60_lr5e4",
    ...
    "A-957/训练策略", "run_queue_30m.py", "DATA_LEGACY", 60, 20, 8, None,
    "LR/调度/预算 策略对比")
...
_ARCH_OLD = ["lg_D1_join_nofpn_notrans", "lg_D2_decoder_ca", "lg_D3_ch_tiny",
             "lg_D4_ch_large"]
reg(_ARCH_OLD, "lg-old/架构(旧管线)", "run_queue_arch.py(v1)", "DATA_NEWSPLIT2",
    60, 20, 8, 2e-4, "P-PNG: 与 nd_* 同管线同协议, 可比较")
reg(["lg_bs8_full_lr2e4"], "lg-old/架构基线", "run_queue_arch.py(v1)", "DATA_NEWSPLIT2",
    60, 20, 8, 2e-4, "P-PNG")
reg(["lg_bs16_full_lr4e4"], "lg-old/架构基线", "run_queue_batch.py", "DATA_NEWSPLIT2",
    60, 20, 16, 4e-4, "P-PNG; bs16 lr 线性缩放")
reg(["lg_b15_full", "lg_b15_noecsam", "lg_b15_noemr", "lg_b15_unet", "lg_b15_deeplab"],
    "lg-old/预算攻击(15轮)", "run_queue_interaction.py", "DATA_NEWSPLIT2", 15, 15, 8, 2e-4,
    "P-PNG")
reg(["lg_const_full", "lg_const_unet", "lg_const_deeplab"],
    "lg-old/恒定LR", "run_queue_interaction.py", "DATA_NEWSPLIT2", 60, 60, 8, 1e-4, "P-PNG")

_lgF = ["lgF_D1_join_nofpn_notrans", "lgF_D2_decoder_ca", "lgF_D3_ch_tiny",
        "lgF_D4_ch_large", "lgF_bs8_full_lr2e4", "lgF_bs16_full_lr4e4"]
reg(_lgF, "lgF-det/架构(确定性增强)", "run_queue_arch.py(v2)", "DATA_NEWSPLIT2",
...
reg(_lgR[:1], "lgR-roll/架构基线", "run_queue_arch.py(v3)", "DATA_NEWSPLIT2", 60, 20, 8, 2e-4,
    "★ 完整模型 6.102M 同管线对照", pipeline="P-MEM-ROLL")
reg(_lgR[1:], "lgR-roll/架构消融与容量", "run_queue_arch.py(v3)", "DATA_NEWSPLIT2",
    60, 20, 8, 2e-4, "P-MEM-ROLL", pipeline="P-MEM-ROLL")
reg(["lgR_b15_full", "lgR_b15_noecsam", "lgR_b15_noemr", "lgR_b15_unet", "lgR_b15_deeplab"],
    "lgR-roll/预算攻击(15轮)", "run_queue_arch.py(v3)", "DATA_NEWSPLIT2", 15, 15, 8, 2e-4,
    "P-MEM-ROLL", pipeline="P-MEM-ROLL")
for _n in (250, 500, 1000):
    reg([f"lgR_n{_n}_{m}" for m in ("full", "noecsam", "unet", "deeplab")],
        f"lgR-roll/数据阶梯 n{_n}", "run_queue_arch.py(v3)", f"data_ladder/n{_n}",
        30, 30, 8, 2e-4,
        "验证集固定为 newsplit2/val; fast_data=False(PNG 子集)", pipeline="P-MEM-ROLL")
reg(["lgV_D1_rollback"], "lgR-roll/回退验证", "verify_rollback.py", "DATA_NEWSPLIT2",
