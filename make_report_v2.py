...
**注**：批量大小（batch）未作调整。显存实测表明 light 模型 batch=16 峰值
4.61 GB（安全），batch=32 需 9.16 GB（超出 8 GB 会 OOM）；且优化后 GPU
利用率已达 84%，继续增大 batch 的边际收益有限，而改变 batch 需按线性缩放
规则重调学习率、会破坏与已完成实验的可比性，故保持 batch=8。
