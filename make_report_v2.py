...
| **D2** | 解码器**无跳连**，仅凭 8× 下采样特征上采样 8 倍 | 前向代码路径 + 分辨率链追踪 | 高分辨率特征（256²/128²/64²）**从未到达输出**；有效空间粒度 8×8 像素 |
...
输出 256² 上每个 **8×8 像素块**的预测，完全由 32² 表征中的**一个 token** 决定。
有效空间粒度 = **8 像素**（@0.3 m ≈ **2.4 m 地面**），即在结构上无法表达比
2.4 m 更精细的边界。
